...
def register():
    ...
    if register_form.validate_on_submit():
        ...
        if result:
            flash("Your Email is already registered, Please try login")
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        result = db.session.execute(db.select(User).where(User.email == email)).scalar()
        if not result:
            flash("Your Email is not registered, Please register")
            return redirect(url_for("register"))
        elif not check_password_hash(result.password, password):
            flash("Invalid please check your password and try again")
        else:
            login_user(result)
            return redirect(url_for("get_all_posts"))
